    ...
    function ignoreDemoTitles() {
        ...
        for ( const [i, p] of packages.entries() ) {
            ...
            if (removeLink) {
                ...
                const isDemo = (name.search(/\b(demo|prologue|trial|episode|alpha|beta|sample|part|trailer|демо|пролог|эпизод|альфа|бета|тест|пробная)\b|(体験|試用|デモ|ベータ|アルファ|序章|试玩|試玩|体验|體驗|演示|前編|前篇|체험|프롤로그|에피소드|알파|베타)(版|판)?|お試し/i) > -1);
                if (isDemo) {
                    games.push(name);
                    removeLink.click();
                }
